...
const gcodeBody = new FormData();
gcodeBody.set("name", `E2E sliced job ${stamp}`);
gcodeBody.set("customerId", customer.id);
gcodeBody.set("file", new Blob(["; generated by OrcaSlicer\n; estimated printing time = 1h 15m\n; filament used [g] = 21.5\nG1 X10 Y10 E1\n"], { type: "text/x.gcode" }), `e2e-${stamp}.gcode`);
const slicedModel = await request("/api/models/upload", { method: "POST", headers: { authorization: `Bearer ${login.accessToken}` }, body: gcodeBody }, 201);
assert.equal(slicedModel.estimatedPrintMinutes, 75);
assert.equal(slicedModel.estimatedFilamentGrams, 21.5);
...
const note = await request(`/api/orders/${order.id}/events`, { method: "POST", headers, body: JSON.stringify({ title: "Проверка качества", message: "Геометрия проверена", isPublic: true }) }, 201);
assert.equal(note.eventType, "NOTE");
const photoBody = new FormData();
photoBody.set("caption", "Первый слой");
photoBody.set("isPublic", "true");
photoBody.set("file", new Blob([Buffer.from("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAusB9YKXaS8AAAAASUVORK5CYII=", "base64")], { type: "image/png" }), "first-layer.png");
const photo = await request(`/api/orders/${order.id}/photos`, { method: "POST", headers: { authorization: `Bearer ${login.accessToken}` }, body: photoBody }, 201);
...
assert.ok(publicOrder.events.some((item) => item.title === "Проверка качества"));
assert.ok(publicOrder.photos.some((item) => item.caption === "Первый слой"));
const publicPhoto = await fetch(`${baseUrl}/api/public/track/${order.trackingCode}/photos/${photo.id}`);
