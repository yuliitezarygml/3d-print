...
type Model={id:string;name:string;originalFilename:string;format:string;fileSizeBytes:number;dimensionsXmm?:number;dimensionsYmm?:number;dimensionsZmm?:number;volumeCm3?:number;triangleCount?:number;estimatedPrintMinutes?:number;estimatedFilamentGrams?:number;version:number;customerId?:string;customerName?:string;previewUrl?:string;fileUrl:string};
...
export default function Models(){
  ...
  return <>
    <PageHeader eyebrow="БИБЛИОТЕКА" title="3D-модели клиентов" description="STL, OBJ, 3MF и G-code привязаны к клиенту, доступны в заказе, Telegram и для скачивания." actions={<label className="customer-filter"><User size={15}/><select value={customerId} onChange={event=>{setCustomerId(event.target.value);setSelected(null)}}><option value="all">Все библиотеки</option><option value="shared">Общие модели</option>{customers.data?.map(customer=><option key={customer.id} value={customer.id}>{customer.name}</option>)}</select></label>}/>
    <div className="upload-zone"><input id="model-file" type="file" accept=".stl,.obj,.3mf,.gcode,.gco" onChange={event=>{const file=event.target.files?.[0];if(file)upload.mutate({file,owner:customerId})}}/><label htmlFor="model-file"><span className="upload-icon"><FileUp/></span><h3>{upload.isPending?"Загружаем и анализируем…":"Добавить модель в выбранную библиотеку"}</h3><p>STL, OBJ, 3MF, G-code · до 200 МБ · {customerId==="all"||customerId==="shared"?"общая библиотека":customers.data?.find(customer=>customer.id===customerId)?.name}</p></label>{upload.error&&<ErrorNote error={upload.error}/>}</div>
