...
func TestAnalyseSlicerEstimateFromGCode(t *testing.T) {
	path := filepath.Join(t.TempDir(), "fixture.gcode")
	gcode := "; generated by OrcaSlicer\n; estimated printing time = 1h 23m\n; filament used [g] = 42.75\n; printer_model = Bambu Lab A1\nG1 X10 Y10 E1\n"
	if err := os.WriteFile(path, []byte(gcode), 0600); err != nil {
		t.Fatal(err)
	}
	minutes, grams, metadata := analyseSlicerEstimate(path, ".gcode")
	if minutes == nil || *minutes != 83 {
		t.Fatalf("minutes = %v, want 83", minutes)
	}
	if grams == nil || math.Abs(*grams-42.75) > 0.001 {
		t.Fatalf("grams = %v, want 42.75", grams)
	}
	if string(metadata) == "{}" {
		t.Fatal("expected slicer metadata")
	}
}

func TestValidGCodeSignature(t *testing.T) {
	if !validModelSignature(".gcode", []byte("; header\nG28\nG1 X10 Y10")) {
		t.Fatal("valid G-code was rejected")
	}
	if validModelSignature(".gcode", []byte("this is not machine code")) {
		t.Fatal("plain text was accepted as G-code")
	}
}
